...
const updateProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (updatedProfile) {
      res
        .status(201)
        .json({ MSG: "Update Successful", updated_Profile: updatedProfile });
    } else {
    ...
    res
      .status(401)
      .json({ MSG: "Error updating user profile", erorr: err.message });
  }
};
...
const deleteUser = async (req, res, next) => {
  ...
  try {
    ...
    if (deletedUser.deletedCount > 0) {
      res
        .status(201)
        .json({ MSG: "Your profile has been deleted from Dubizzle" });
    } else {
      res.status(401).json({ MSG: "User not found" });
    }
  } catch (err) {
...
const addProductToFavourite = async (req, res, next) => {
  try {
    ...
    if (productIndex === -1) {
      ...
      return res
        .status(500)
        .json({ MSG: "This Product is already in your Favorites" });
    }
  } catch (error) {
    return res
      .status(500)
